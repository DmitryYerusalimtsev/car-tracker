...
final class CarRouteConfig(system: ActorSystem) {
  ...
  private val carsManager = system.actorOf(CarsManager.props(IgniteTelemetryRepository()), "cars-manager")
  ...
  def route(): Route = {
    pathPrefix("car") {
      get {
        path("alltelemetry") {
          val getTelemetryFuture = (carsManager ? CarsManager.GetAllCarTelemetry(UUID.randomUUID()))
